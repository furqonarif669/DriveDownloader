...
* **Secure Authentication Token**: Google Drive tokens are stored only in system RAM. If you close the app or unplug the USB, the session is wiped instantly.
...
1.  **Launch the App**: Double-click `DriveDownloader.exe` (Windows) or `DriveDownloader.app` (Mac).
2.  **Login**: Click **"Login with Google"**. A browser window will open. Authorize the app.
    * *Note: Your login session exists only while the app is open.*
3.  **Paste Link**: Copy a Google Drive folder or file link and paste it into the input box.
4.  **Download**: Click **Download**, select a destination folder on your computer/USB, and watch it go!
5.  **Manage**: You can Pause, Resume, or Delete tasks from the list.
...
```Bash
xattr -cr /path/to/DriveDownloader.app
```